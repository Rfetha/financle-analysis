...
- **Chat (UC7):** LangGraph `create_react_agent` — **dinamik** tool seçimi; `SqliteSaver`
  ile thread memory (aynı SQLite, ADR-0004). **Tek loop:** model katmanı hangi sağlayıcıyı
  verirse versin döngü LangGraph'tadır (ADR-0002; Claude Code SDK yolu 2026-07-13'te söküldü).
...
**Tool katmanı paylaşımlı:** aynı 13 deep tool iki tüketiciye açık — ReAct'e (`@tool`) ve
recipe'lere (doğrudan Python çağrısı). **MCP server / "harici beyin" yolu düştü** (2026-07-13):
Sonar kendi akıllı uygulaması, beynini dış istemciye ödünç vermez.
...
- Beyin uygulamanın içinde: model katmanı (local / OpenRouter / API key — ADR-0002).
